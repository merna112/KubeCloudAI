    ...
    <div className='bg-gray-50 dark:bg-gray-900 min-h-screen flex flex-col'>
      <header className='relative bg-gradient-to-r from-teal-500 to-teal-600 text-white py-10 sm:py-12 md:py-16 overflow-hidden'>
        <div className='w-full max-w-screen-xl mx-auto px-4'>
          <div className='relative h-full flex items-center justify-center' style={{ minHeight: '6rem' }}>
            <div
              className='flex flex-nowrap transition-transform duration-1000 ease-in-out'
              style={{ transform: `translateX(-${slideIndex * 100}%)` }}
            >
              {slides.map((text, index) => (
                <div key={index} className='flex-shrink-0 w-full text-center flex items-center justify-center px-2'>
                  <h1 className='text-xl xs:text-2xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-6xl font-extrabold break-words leading-tight'>
                    {text}
                  </h1>
                </div>
              ))}
